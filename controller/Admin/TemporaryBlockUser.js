...
async function TemporaryBlockUser(req, res) {
  ...
  connection.query(
    'SELECT TemporaryBlock FROM Users WHERE Id = ?',
    [userId],
    (err, results) => {
      if (err) {
      ...
      const currentStatus = results[0].TemporaryBlock;
      const newStatus = currentStatus === 1 ? 0 : 1;

      // Update the user's TemporaryBlock status
      connection.query(
        'UPDATE Users SET TemporaryBlock = ? WHERE Id = ?',
        [newStatus, userId],
        (updateErr) => {
          if (updateErr) {
          ...
          return res.status(200).json({ 
            message: `User ${newStatus === 1 ? 'temporarily blocked' : 'unblocked'} successfully`,
